...
public class CheckpointEntry {
     ...
     * <p><b>这只是降低概率，不是消除</b>：保留旧 mtime 的替换方式（`cp -p`、robocopy /COPY:T、
     * 从备份还原）依旧能通过守卫；要真正确认只能靠传输后 MD5 校验（`-x`）。</p>
     * <p>已知假阳性（方向安全，只是退化为全量重传）：① 源文件 mtime 晚于接收端当前时间（两端时钟偏差）；
     * ② 进程被强杀/断电，最后一次写入晚于节流存档。</p>
     */
