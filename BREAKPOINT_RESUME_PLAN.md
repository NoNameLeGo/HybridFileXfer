...
> 状态：断点续传 / 进度显示 / MD5 校验 / 文件名清洗（P4）四项均已实现
> 目标：为 HybridFileXfer 增加块级断点续传、传输后文件校验、整体进度显示
>
> ⚠️ 实现与本文档的差异、以及审查发现的缺陷，见 [`CODE_REVIEW_FINDINGS.md`](CODE_REVIEW_FINDINGS.md)。
> 主要差异：检查点持久化单位由「块数」改为「字节偏移」；MD5 由「流式累加」改为「传输后重读磁盘计算」。
...
> #109 已于 2025-10-30 以 COMPLETED 关闭且是**误报**（发帖人自述为手机可变帧率导致剪辑软件解析错误，
> MD5 校验全部通过），不再作为「写入验证」的需求依据；#75 亦已关闭。
...
| **校验开关** | **可选项，默认关闭** | 校验有计算与跳过块补偿读取成本；由发起方在传输前选择开启（Android 确认对话框 Switch / PC `--checksum` 参数），偏好持久化。校验开启时跳过块也需计入哈希（发送方读本地文件、接收方读磁盘已有数据补齐），进程内累积即可 |
