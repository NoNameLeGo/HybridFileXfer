...
本仓库是 [weixiansen574/HybridFileXfer](https://github.com/weixiansen574/HybridFileXfer) 的 fork，主要增加（按发布顺序）：

**v3.0.1 / v3.0.3**：块级断点续传、总体进度（手机端百分比 + 电脑端单行刷新）、
`-x/--checksum` 传输后校验。

> ⚠️ **别在生产上依赖这两个版本的续传**：301 协议的续传握手字段不对称（会卡死控制通道），
> 且检查点记的是「最后落盘块的末尾」而不是连续水位线，多通道乱序下会跳过未到达的块、静默丢数据。

**v3.0.4（2026-09-15，推荐）**：

- 修正上面两个缺陷：连续水位线检查点 + 握手字段对称（协议升到 **303**）
...
> ⚠️ **协议版本**：v3.0.4 是协议 **303**，更早的 v3.0.1/v3.0.3 是 **301**，
> 两者**不能互连**（会直接报版本不一致）。手机端 APK 与电脑端 jar 必须来自同一版本。
> （源码 HEAD 已升到 304：校验期间发心跳，尚未发版。）
>
