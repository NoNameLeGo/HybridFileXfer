...
本仓库是 [weixiansen574/HybridFileXfer](https://github.com/weixiansen574/HybridFileXfer) 的 fork。

**已发版（v3.0.1 / v3.0.3）**：

- **块级断点续传**、**总体进度**（手机端百分比 + 电脑端单行刷新）
- **传输后校验**：`-x/--checksum` 开启，传完双方各自重读磁盘算 MD5 并比对

> ⚠️ 但**别在生产上依赖已发版本的续传**：301 协议的续传握手字段不对称（会卡死控制通道），
> 且检查点记的是「最后落盘块的末尾」而不是连续水位线，多通道乱序下会跳过未到达的块、静默丢数据。

**已合并入源码但尚未发版**（最新 Release 停在 v3.0.3）：

- 续传两个缺陷的修正：连续水位线检查点 + 握手字段对称（协议升到 **303**）
- **文件名清洗**：`:` `?` `*` 等 Windows 非法字符、尾部点/空格、`CON`/`NUL` 等保留设备名、
  超长文件名不再导致接收端崩溃（清洗撞车时自动加序号）
- 校验失败时列出**具体文件名**；控制通道读超时看门狗
- 长文件名不再 crash（#113）、缓冲区块过多不再把 JVM 崩掉（#84）

> ⚠️ **协议版本**：Release 里的 v3.0.1/v3.0.3 是协议 **301**，当前源码 HEAD 是 **303**，
> **两者不能互连**（会直接报版本不一致）。手机端 APK 与电脑端 jar 必须来自同一版本：
> 要用上述未发版改动，需自己构建两端（Android 端交给 GitHub Actions 的 Build 任务）。
>
> ⚠️ **签名不同**：本 fork 使用独立发布签名，与上游版本**互不覆盖安装**，从上游切过来需先卸载。
...
主界面选择好模式后点击按钮`启动服务器并等待连接`，状态显示“等待连接”后，电脑双击运行启动脚本`start-by-adb.bat`（`script/` 目录下；不想用 ADB 就用 `start-by-network.bat`，或者你根据启动参数自己编写一个）。
如果提示没有 java，则需要[安装java运行环境](https://www.oracle.com/java/technologies/javase/jdk17-archive-downloads.html)，安装过程不过多阐述，与我的世界安装java环境过程一致。ADB连接方式下，程序首先执行adb转发端口（USB通道），若电脑未授权USB调试，请点击手机上的“允许这台电脑进行调试”。端口转发完成后，启动电脑客户端，客户端先连接控制通道，然后从控制通道获取手机指定的传输通道网卡信息，然后电脑客户端连接到这些传输通道网卡，选择的线路都连接成功后，『传输文件』按钮亮起即可进行文件传输。
...
java -jar HybridFileXfer.jar -c 192.168.1.2 -x     # 传完做一次 MD5 校验
```
...
> 注意：该 AUR 包拉取的是**上游仓库**的源码，不是本 fork，因此没有断点续传 / 传输后校验 / 文件名清洗。
...
从 3.0.1（本 fork）起有了总体进度：手机端传输对话框显示百分比与已传/总字节，电脑端命令行单行刷新。

需要注意的是，发送方报的是「已读入缓冲队列」的字节数，而不是「已到达对端」——缓冲池有界（默认一百多 MB），
所以这个滞后是有界的，但网络卡住时进度会比实际略快一点。传输完成后可加 `-x` 让双方重算 MD5 做最终校验。
...
[![Star History Chart](https://star-history.dera.page/svg?repos=weixiansen574/HybridFileXfer&type=Date)](https://star-history.dera.page/#weixiansen574/HybridFileXfer&Date)
