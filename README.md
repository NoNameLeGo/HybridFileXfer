...
**v3.0.5（2026-09-15，最新，协议 304）**：

- 修一处控制通道并发写（PC 用 `-x` 且从手机上传时，刷新远端列表与校验交换会抢写同一条通道）
- 传输耗时 0ms 时的除零崩溃（PC 会带栈退出）
- 文件名按 UTF-8 **字节**截断（Linux/Android 是 255 字节而非 255 字符，中文长名之前仍然失败）
- 检查点 mtime 守卫（目标文件被替换后不再跳字节续写）、校验期间的心跳（单个大文件算 MD5
  超过 10 分钟不再被看门狗误杀）、对传时的校验开关
- 一轮代码审查的健壮性修复：fd/socket/缓冲块泄漏、校验清单为空时不回结果、写失败时丢缓冲块等

> ⚠️ **协议版本**：v3.0.5 = **304**，v3.0.4 = **303**，v3.0.1/v3.0.3 = **301**，
> 三者**不能互连**（会直接报版本不一致）。手机端 APK 与电脑端 jar 必须来自同一版本。
>
