...
**这是破坏性协议变更：`VERSION_CODE` = 304，手机端必须同步升级 APK**，否则连接时会明确报版本不一致
（旧版行为是控制通道错位死锁）。

**已于 2026-09-15 发布 `v3.0.5`（协议 304，最新）**：一轮代码审查的修复（校验期间控制通道并发写、除零、
文件名按 UTF-8 字节截断、检查点 mtime 守卫、校验心跳、fd/socket/缓冲块泄漏等）+ 对传时的校验开关。
手机端与 PC 端需同时用 v3.0.5（或同时用旧版），不能混。

更早的 `v3.0.4`（协议 303）含断点续传加固、文件名清洗、校验失败文件名清单、读超时看门狗、#113/#84 修复，
不再发 antutu-turbo 变体。
...
3. 校验看门狗的 10 分钟无进展阈值未在真机长传输上验证（正常大文件 MD5 不应触发）。
   本次把应答方自己的看门狗也接上了心跳（`watchdog.touch()` + 发帧），但**没有自动化测试**：
   要测需要可注入的超时阈值（现在写死在 `CHECKSUM_STALL_TIMEOUT_MS`）。
4. 检查点 mtime 守卫（`目标文件 mtime <= entry.timestamp`）在真机上的假阳性：
   两端时钟偏差、强杀/断电后重连、以及「换连接方式续传」是否仍能命中（预期：命不中则全量重传，不损坏数据）。
