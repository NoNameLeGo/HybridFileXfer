...
var exitCode = await LaunchJavaAsync(jarFileName, jarPath, args);

// 双击启动时控制台会随进程退出立即关闭：未带参数（只打印帮助）或 java 异常退出时
// 等待按键再关窗，让用户来得及阅读输出
if ((exitCode != 0 || args.Length == 0) && !Console.IsInputRedirected) {
	Console.WriteLine("\n按任意键关闭窗口……");
	Console.ReadKey(intercept: true);
}
return exitCode;

static async Task<int> LaunchJavaAsync(string jarFileName, string jarPath, string[] args) {
	// 检查 .jar 文件是否存在
